...
const LB_BLOCKS = [
  { key: 'lookbook.hero', title: 'Portada del lookbook', kind: 'lb-hero',
    help: 'Apertura del lookbook. En una marca sobre papel es un índice tipográfico sin foto: las seis entradas numeradas salen solas de las historias; el título es el nombre de la temporada (p. ej. «SS27»): no se pinta, da nombre a la apertura para lectores de pantalla («Lookbook SS27»); el subtítulo es la línea de estado que cierra el índice y debe nombrar la temporada, porque el título no se pinta («SS27 · Abierta a pedidos.», con fechas si la campaña las tiene) y el botón la acción a su lado (vacío = «Ver todo el catálogo», la misma etiqueta que el cierre); el kicker solo si se quiere un rótulo encima del índice (normalmente vacío: la pestaña LOOKBOOK ya rotula la página); la imagen se ignora y puede quedar vacía. En las demás marcas, con imagen es un carrusel (2880×1200 px o mayor).' },
